...
echo "*********************************************************************************************************"
sleep 5
wget -O Azeddine.txt http://www.proxyserverlist24.top/
echo "*********************************************************************************************************"
grep "href='http://www.proxyserverlist24.top/2019" Azeddine.txt | cut -d"'" -f2 | grep -v  more | grep -v link > links.txt
rm Azeddine.txt
cat links.txt | sed -n '2p' > 1links.txt
for a in `cat 1links.txt`
do
wget -O Azeddine2.txt $a
done
cat Azeddine2.txt | grep ":" | cut -d":" -f1,2 | grep -v [a-Z] > proxy_list
rm Azeddine2.txt
rm links.txt
rm 1links.txt
clear
